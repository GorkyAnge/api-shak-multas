...
const crearMulta = async (req, res) => {
  try {
    ...
    res.status(201).send({ id: multaRef.id, mensaje: 'Multa creada con éxito' });
  } catch (error) {
...
const pagarMulta = async (req, res) => {
  try {
    ...
    if (response.status === 200) {
      ...
      res.send({ mensaje: 'Pago realizado con éxito', response: response.data });
    } else {
      res.status(400).send({ mensaje: 'Error en el pago', response: response.data });
    }
  } catch (error) {
...
const obtenerMultas = async (req, res) => {
  try {
    const { id } = req.params;
    const multasSnapshot = await db.collection('multas').where('id', '==', id).get();
    const multas = [];
